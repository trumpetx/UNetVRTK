...
namespace VRTK
{
    ...
    public class VRTK_RadialMenu : MonoBehaviour
    {
        ...
        [Tooltip("When the menu grows from a hidden state, this is the target scale.")]
        public float targetGrowScale = 1;
        [Tooltip("How fast to transition between hidden and shown (bigger = faster, relates to scale)")]
        public float targetGrowTime = 4f;
            ...
            for (int i = 0; i < buttons.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (!rotateIcons)
                    {
                        ...
                        buttonIcon.transform.eulerAngles = parentCanvas.transform.eulerAngles;
                    }
                }
        ...
        protected virtual IEnumerator TweenMenuScale(bool show)
        {
            ...
            {
                transform.localScale += Dir * Time.deltaTime * targetGrowTime; //Tweening function - currently 0.25 second linear
                yield return true;
                i++;
            }
